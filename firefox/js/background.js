...
browser.storage.sync.get(null).then(function (conf) {
    var newConfig = {};
...
function sendRequest(request) {
    ...
    browser.storage.sync.get([YTT_CONFIG_USERID, YTT_CONFIG_FAILED_SHARE]).then(function (config) {
...
function playerStateChange(event) {
    if (event[YTT_STATE_EVENT_STATE_KEY] === '1') {
        ...
        browser.storage.sync.get([YTT_CONFIG_REAL_TIME_KEY, TODAY_KEY, YTT_CONFIG_SHARE_ONLINE]).then(function (config) {
...
function setVideoDuration(event) {
    var TODAY_KEY = YTTGetDayConfigKey();
    browser.storage.sync.get([YTT_CONFIG_IDS_WATCHED_KEY, YTT_CONFIG_START_TIME_KEY, YTT_CONFIG_TOTAL_TIME_KEY, TODAY_KEY, YTT_CONFIG_SHARE_ONLINE]).then(function (config) {
        var toRemove = [];
...
browser.storage.sync.get([YTT_CONFIG_USERID, YTT_CONFIG_DEBUG_KEY]).then(function (config) {
